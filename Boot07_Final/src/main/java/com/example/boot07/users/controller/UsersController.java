...
public class UsersController {
	...
	public Map<String, Object> profileUpload(HttpServletRequest request, MultipartFile image) {
		// 서비스를 이용해서 이미지를 upload 폴더에 저장하고 리턴되는 Map을 리턴해서 json 문자열 응답
	...
	public String pwdUpdate(UsersDto dto, Model model, HttpSession session) {
		...
		// view page로 forward 이동해서 작업 결과 응답
		return "users/pwd_update";
	}
	
	// 비밀번호 수정 폼 요청 처리
	@GetMapping("/users/pwd_updateform")
	public String pwdUpdateFrom() {
		return "users/pwd_updatefrom";
	}
	...
	public String login(Model model, UsersDto dto, String url, HttpSession session) {
		/*
			...
			주로 HttpServletRequest, HttpServletResponse, HttspSession, ModelAndView
			등의 객체이다.
		*/
		service.loginProcess(dto, session);
		
		// 로그인 후에 가야 할 목적지 정보를 인코딩 하지 않는 것과 인코딩 한 것을 모두 ModelAndView 객체에 담는다.
